...
class FitTextBlock {
    ...
    clearBlock() {
        ...
        //this.size = `${document.documentElement.clientWidth}x${document.documentElement.clientHeight}-${Css.get('--writing-mode')}`
        this.#setSize()
    ...
    #setSize() { this.size = `${document.documentElement.clientWidth}x${document.documentElement.clientHeight}-${Css.get('--writing-mode')}` }
...
class Pager {
    ...
    setSplitterText() {
        ...
        layoutScreen.Noble.set(this.page+1, Math.ceil(this.logs.length / this.#screenCount()))
    }
}
class FitElement {
