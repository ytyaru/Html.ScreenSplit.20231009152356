class LayoutScreen {
    constructor() { this.el=null; this.options=null; this.splitter=new Splitter(); this.inners=[]; }
    ...
    #addInnerScreen() {
        for (let i=0; i<this.options.count; i++) {
            ...
            */
            this.el.appendChild(this.#makeInnerScreen(i))
        }
        console.debug('addInnerScreen:', this.options.count)
    }
    #makeInnerScreen(i) {
        if (i < this.inners.length-1) { return this.inners[i] }
        const inner = document.createElement('div')
        inner.classList.add('inner-screen')
        this.inners.push(inner)
        return inner
    ...
    #insertCenterSplitter() {
        console.log('#insertCenterSplitter()')
        if (!this.#hasSplitter()) { return }
        console.log('#insertCenterSplitter() 2')
        const splitter = this.el.querySelector('.splitter')
        console.log(splitter)
        console.log(splitter || 'Oops!!')
//        console.log(((splitter) ? splitter : this.splitter.make()))
        this.el.querySelector('.inner-screen').insertAdjacentElement('afterend', ((splitter) ? splitter : this.splitter.make()))
//        this.el.querySelector('.inner-screen').insertAdjacentElement('afterend', splitter || this.splitter.make())
...
//    #gridTemplateRows() { return `${this.#clientBlock() - this.#fontSize()}px` + ((1===this.options.count) ? ` 1fr` : '') }
    ...
    #setWritingMode() {
        if (this.#isVertical() || this.#isHorizontal()) {
            ...
            Css.set('--text-decoration', 'underline') // underline(下,右), overline(上,左) : 書字方向(縦,横)
//            Css.set('--splitter-writing-mode', this.#writingModeReverse())
    ...
    setSplitterText(text) {
        ...
        if (el) { el.innerHTML = `<div class="loader book"><figure class="page"></figure><figure class="page"></figure><figure class="page"></figure></div><span>${text}</span>` }
        //if (el) { el.innerHTML = `<div class="solo-icon loading-icon-circle"></div><span>${text}</span>` }
        //if (el) { el.textContent = text }
        //if (el) { el.textContent = `中央スプリッター　1/${this.logs[this.size].length}` }
    }
    toggleWritingMode() {
        this.options.writingMode = (this.#isVertical()) ? 'horizontal' : 'vertical'
        this.remake()
        //if (1!==this.options.count && 2!==this.options.count) { this.options.auto=true; this.options.count=this.#calcCount(); }
        //else { this.options.auto=false }
        //this.resize()
