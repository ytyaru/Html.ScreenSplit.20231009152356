...
class FitElement {
    ...
    addBlock(block) {
        ...
        if ('p'===blockEl.tagName.toLowerCase()) {
            ...
            if (!children) { return }
            // テキストノードspanをさらに文単位句点。ごとに分割する
            children = children.map(el=>this.spanner.splitSentence(el)).flat()
            children = this.#addElements(children)
            if (!children) { return }
//            // 一字ずつspanにする
...
//            if (!children) { return }
    ...
    #addElements(children) {
        for (let i=0; i<children.length; i++) {
            ...
            if (!res) { return children.slice(i) }
        }
    }
    #addInline(children) {
        console.log('#addInline()', children)
        let ok = 0
        for (let i=0; i<children.length; i++) {
            const res = dummyScreen.addInline(children[i])
            if (!res) {
                if (0===ok) { throw new Error('最小単位一つさえ一画面に収まりません。') }
                this.logs.push({'blockStartIndex':this.startIndex, 
                                'blockEndIndex':this.startIndex, 
                                'html':dummyScreen.screen.innerHTML})
                dummyScreen.clear()
